...
// get a distance reading from the USS
  double test = USS_measure(PIN_TRIG,PIN_ECHO);
  dist_raw = (test!=0)? test: dist_raw;
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw<200) {
    double LedLight = 255-(dist_raw-100)*(2.55); 
    analogWrite(PIN_LED, int(LedLight));
  }
  else if (dist_raw==300 || dist_raw==100){
    ...
    double LedLight = 255-(300-dist_raw)*(2.55); 
    analogWrite(PIN_LED, int(LedLight));
  }
  
// do something here
  // delay(50); // delete delay(50);
